...
def Teams(request):
    main_team = Team.objects.filter(isMain=True)[0]
    teams = Team.objects.filter(isMain=False)
    main_team_members = Member.objects.filter(team=main_team.id)
    print teams
    return render_to_response("team.html", {"main_team": main_team, "teams": teams,
                                            "main_team_members": main_team_members},
                              context_instance=RequestContext(request))


def Members(request, id):
    response_data = dict()
    members = Member.objects.filter(id=int(id))
    if len(members) == 0:
        ...
        response_data["message"] = members[0].toDict()
        return HttpResponse(json.dumps(response_data), content_type="application/json")
...
def List(request, type):
    ...
    for announcement in announcements:
        response_data["message"].append({"title": announcement.title, "editor": announcement.editor.username,
                                         "submittime": announcement.submittime.strftime("%Y-%m-%d %H:%M:%S"),
                                         "id": announcement.id})
